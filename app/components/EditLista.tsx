...
export default function Taskitems({ id, tarefa, editando, setedit }: { id: number, tarefa: string, editando: number | null, setedit: Dispatch<SetStateAction<number | null>> }) {
    ...
    return (
        <>
            {editando === id ? <div className="flex items-center gap-2 max-w-sm mt-2"><Input value={inputedit} onChange={(e) => setinput(e.target.value)} placeholder="Qual vai ser o novo valor?" /> <Button onClick={cancel} size="icon" className="bg-[#F87171] hover:bg-[#8b0000] rounded-sm" title="Descartar alteração"><Undo color="#ffff"/></Button><Button onClick={ConfirmEdit} size="icon" className="rounded-sm bg-[#267D39] hover:bg-[#30543D]" title="Confirme a alteração"><Check color="#ffff"/></Button> </div> :
                <>
                    <span className="flex-1 min-w-0 text-sm text-zinc-300 truncate" title={tarefa}>
                        {tarefa}
                    </span>
