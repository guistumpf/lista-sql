...
export default function Taskitems({
  ...
  function cancel() {
    const confirmed = confirm(
      `Tem certeza? As alterações serão canceladas. Valor Atual: ${tarefa}`,
    );
    if (confirmed) {
      setedit(0);
    }
      ...
      {editando === id ? (
        <div className="flex items-center gap-2 max-w-sm mt-2">
          <Input
            ...
            placeholder="Qual vai ser o novo valor?"
          />
          <Button
            onClick={cancel}
            size="icon"
        ...
        <>
          <span
            className="flex-1 min-w-0 text-sm truncate text-foreground"
            title={tarefa}
          >
            {tarefa}
          </span>
